...
# Global vehicle count dictionary for API access
live_vehicle_counts = {"motorcycle": 0, "car": 0, "truck": 0, "bus": 0, "bicycle": 0, "tricycle": 0}
...
# Redis setup for live vehicle count sharing
import redis
...
def inference_worker():
    ...
    while not stop_inference:
        try:
            ...
            persons, motorcycles = [], []
            vehicle_types = ["motorcycle", "car", "truck", "bus", "bicycle", "tricycle"]
            ...
            print(f"[VEHICLE COUNT] Frame {local_frame_count}: " + ", ".join([f"{vt}: {vehicle_counts[vt]}" for vt in vehicle_types]))
            # Update global live_vehicle_counts for API
            for vt in vehicle_types:
                live_vehicle_counts[vt] += vehicle_counts[vt]
            ...
            # Push live vehicle counts to Redis
            try:
                redis_client.hmset("live_vehicle_counts", vehicle_counts)
            except Exception as e:
