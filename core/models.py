...
class VehicleCount(models.Model):
    camera = models.ForeignKey(Camera, on_delete=models.CASCADE)
    timestamp = models.DateTimeField(auto_now_add=True)
    motorcycle = models.PositiveIntegerField(default=0)
    car = models.PositiveIntegerField(default=0)
    truck = models.PositiveIntegerField(default=0)
    bus = models.PositiveIntegerField(default=0)
    bicycle = models.PositiveIntegerField(default=0)
    # Add other classes as needed

    def __str__(self):
        return f"VehicleCount @ {self.timestamp} (Cam: {self.camera.name})"
